...
append_and_read(_) ->
  ...
  {ok, T1, ReadDB2} = pulsedb:read(<<"input">>, [{name,<<"source1">>}, {from, "1970-01-01"},{to,"1971-02-02"}], ReadDB1),
  [{120,5},
   {180,10},
   {240,3},
   {4000140,5},
   {4000200,10},
   {4000260,3}] = [T || {_,V}=T <- T1, V > 0],


  {ok, T2, ReadDB3} = pulsedb:read(<<"input">>, [{name,<<"source1">>}, {from, "1970-01-01"},{to,"1970-01-02"}], ReadDB2),
  [{120,5},
   {180,10},
   {240,3}] = [T || {_,V}=T <- T2, V > 0],

  
  {ok, T3, ReadDB4} = pulsedb:read(<<"input">>, [{from, "1970-01-01"},{to,"1971-01-02"}], ReadDB3),
  [{120,7},
   {180,10},
   {240,3},
   {4000140,5},
   {4000200,14},
   {4000260,3}] = [T || {_,V}=T <- T3, V > 0],
